...
export class PropertyService {
  ...
  getAllProperties(page: number = 0, size: number = 20): Observable<any> {
    return this.apiService.get<any>(`${this.baseUrl}/properties`, {
      page,
      size
    });
  }
  ...
  searchProperties(filters: PropertySearchFilters): Observable<PropertySearchResultDTO[]> {
    ...
    if (filters.amenityIds && filters.amenityIds.length > 0) params.amenityIds = filters.amenityIds.join(',');
